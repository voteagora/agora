...
export function makeNounsNameResolver(
  provider: ethers.providers.Provider
): NameResolver {
  return {
    async resolveAddress(name: string) {
      return (await resolveEnsOrNnsName(name, provider)) as Address | null;
    },
    async resolveNameFromAddress(address: Address) {
